...
class start(threading.Thread):
    ...
    def run(self):
        ...
        while not self.finished.isSet():
            ...
            try:
                ...
                try:
                    data = self.instrument.read_registers(0, 10, functioncode=3) # Registernumber, number of decimals
                    print data
                    h2s = data[2]
                    lel = data[0]

                    if self.last_h2s != h2s:
                        self.sendtodb("H2S", h2s, 0)
                        self.last_h2s = h2s

                    if self.last_lel != lel:
                        self.sendtodb("LEL", lel, 0)
                        self.last_lel = lel
    ...
    def mcx32_signal(self, name, value):
        self.signal()
        return True
        
    def mcx32_sync(self, name, value):
        self.register()
        self.count = 3001
        return True
