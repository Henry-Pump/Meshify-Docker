...
class start(threading.Thread):
    ...
    def __init__(self, name=None, number=None, mac=None, Q=None, mcu=None, companyId=None, offset=None, mqtt=None, Nodes=None):
        ...
        try:
            self.instrument = minimalmodbus.Instrument('/dev/ttyUSB0', 2) # port name, slave address (in decimal)
        ...
        while not self.finished.isSet():
            ...
            try:
                ...
                try:
                    ...
                    print tlevel
                    if tlevel != self.last_tlevel:
                        self.sendtodb("tlevel", str(tlevel), 0)
                    self.last_tlevel = tlevel
                    ...
                    print wlevel
                    if wlevel != self.last_wlevel:
                        self.sendtodb("wlevel", str(wlevel), 0)
                    self.last_wlevel = wlevel
                    ...
                    temp = int(temp)
                    print temp
                    if abs(temp - self.last_temp) > 1:
                        self.sendtodb("temp", str(temp), 0)
                        self.last_temp = temp
