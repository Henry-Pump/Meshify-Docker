...
class m1(threading.Thread):
    ...
    def __init__(self, name, number, mac, q, mcu, company, offset):
    ...
    def run(self):
        ...
        try:
            #print "here is where we send data to the APG sensors"
            self.readAll("hi")
        ...
        while True:
            ...
            try:
                ...
                for item in self.currentDict:
                    print "comparing ", self.currentDict[item], " to ", last[item]
                    if self.currentDict[item] != last[item]:
    ...
    def m1_update(self, name, value):
        try:
            self.currentDict = self.mcu.getDict()
            print "here is the dictionary ", self.currentDict
            self.lastDict = self.currentDict
            for item in self.currentDict:
                self.sendtodb(item, self.currentDict[item], 0)
        except:
            print "couldnt' get the dictionary from the mcu"
