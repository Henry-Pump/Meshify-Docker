...
class gds(threading.Thread):
	...
	self.debug = True
        ...
        self.last_alarm1 = False
        self.last_alarm2 = False
        self.last_alarm3 = False
        self.last_Fault = False
        self.last_LowBat = False
        self.last_ChannelOn = False
        self.last_ChannelCommError = False
        self.lvlDict = False 
        self.nameDict = False
    ...
    def run(self):
        ...
        while not self.finished.isSet():
            try:
                ...
                #self.sendtodbCH(str(1), "a1", "1", 0)

                #self.getNames(40400)
                
                newAlarm1 = self.getBitArray(33999, "a1", self.last_alarm1)
                self.last_alarm1 = newAlarm1
                
                newAlarm2 = self.getBitArray(34001, "a2", self.last_alarm2)
                self.last_alarm2 = newAlarm2
                
                newAlarm3 = self.getBitArray(34003, "a3", self.last_alarm3)
                self.last_alarm3 = newAlarm3
                
                newFault = self.getBitArray(34005, "flt", self.last_Fault)
                self.last_Fault = newFault
                
                newCommErr = self.getBitArray(34027, "nocomm", self.last_ChannelCommError)
                self.last_ChannelCommError = newCommErr
                
                newLowBat = self.getBitArray(34007, "lobatt", self.last_LowBat)
                self.last_LowBat = newLowBat
                
                self.getlevel(33064) #reg = 33064
                
                #self.getNames(40400)

                """
                #self.alarm1()
                #resp = self.read_float(33064, functioncode=4) float
                #resp = self.read_long(33999, functioncode=4)  bitarray
                #resp = self.read_long(34025, functioncode=4)  
                
                resp = self.read_long(40400,functioncode=3)
                print "####### here is the response ", resp
                resp = self.read_long(40402,functioncode=3)
                print "####### here is the response ", resp
                resp = self.read_long(40404,functioncode=3)
                print "####### here is the response ", resp
                resp = self.read_long(40406,functioncode=3)
                print "####### here is the response ", resp
                """
    ...
    def getNames(self, reg):
        channelcount = 1
        if self.nameDict == False:
            self.nameDict = {}
            while channelcount <= 32:
                try:
                    
                    subreg = reg
                    print "############## here is the reg number"
                    print subreg
                    part1 = self.read_string(subreg, numberOfRegisters=2, functioncode=3)
                    print part1
                    #part1 = hex(int(part1)).split('x')[1].decode("hex")

                    
                    subreg += 2
                    part2 = self.read_string(subreg, numberOfRegisters=2, functioncode=3)
                    print part2
                    #part2 = hex(int(part2)).split('x')[1].decode("hex")

                    
                    subreg += 2
                    part3 = self.read_string(subreg,  numberOfRegisters=2, functioncode=3)
                    print part3
                    #part3 = hex(int(part3)).split('x')[1].decode("hex")

                    
                    subreg += 2
                    part4 = self.read_string(subreg, numberOfRegisters=2,  functioncode=3)
                    print part4
                    ...
                    self.sendtodbCH(str(channelcount), "name", value, 0)
                    self.nameDict[channelcount] = value
                except:
            ...
            while channelcount <= 32:
                try:
                    ...
                    value = str(part1) + str(part2) + str(part3) + str(part4)
                    if value != self.nameDict[channelcount]:
                        self.sendtodbCH(str(channelcount), "name", value, 0)
                    self.nameDict[channelcount] = value
                except:
                    print "errer getting name"
    ...
    def getlevel(self, reg): #reg = 33064
        channelcount = 1
        if self.lvlDict == False:
            self.lvlDict = {}
            while channelcount <= 32:
                try:
                    resp = self.read_float(reg, functioncode=4)
                    value = str(resp)
                    
                    self.sendtodbCH(str(channelcount), "lvl", value, 0)
                    self.lvlDict[channelcount] = value
                except:
            ...
            while channelcount <= 32:
                try:
                    resp = self.read_float(reg, functioncode=4)
                    value = str(resp)
                    if value != self.lvlDict[channelcount]:
                        self.sendtodbCH(str(channelcount), "lvl", value, 0)
    ...
    def getBitArray(self, reg, name, lastValue):
        lst = self.read_long(int(reg), functioncode=4)
        lst = int(lst)
    ...
    def channelEnabled(self):
        ...
        lst = self.read_long(34025, functioncode=4)
    ...
    def apgus_relay1(self, value):
        value = int(value)
        self.mcu.relay1(value)
        self.sendtodb("relay1", str(value), 0)
    ...
    def _communicate(self, message):
        ...
        msg = ''.join(map(lambda c:'%02x'%c, map(ord, message)))
        msg = str(msg)
        ...
        self.mcu.last_485 = None
        count = 0
        self.mcu.send_rs485(msg)
        self.first_data = True
        while self.mcu.last_485 == None:
            count += 1
            time.sleep(1)
            if count > 10:
                break
        
        data = binascii.unhexlify(self.mcu.last_485)
        answer =  data
