...
class start(threading.Thread):
    ...
    def run(self):
        ...
        self.sendtodb("version", self.version, 0)
        thread.start_new_thread(self.doorOpenThread, ())
        while True:
            try:
                self.uploadFiles()
            except Exception,e:
    ...
    def uploadFiles(self):
        ...
        for filename in os.listdir("/root/videos"):
            if filename.endswith(".mp4"):
                if int(os.path.getsize("/root/videos/" + filename)) < 20:
                    os.remove("/root/videos/" + filename)
                    continue
                print filename, " Is about to upload"
                if self.status == "idle":
                    self.status = "Door Triggered"
                ...
                try:
                    ...
                    os.remove("/root/videos/" + filename)
                    self.sendtodb("log", "Video Clip Uploaded", 0)
                except Exception,e:
                    os.remove("/root/videos/" + filename)
                    print e
    ...
    def doorOpenThread(self):
        while True:
            if self.mcu.getDict()["din2"] == "Off":
                if self.last_din2 == "On":
                    self.sendtodb("status", "Door Triggered", 0)
