...
class XMODEM(object):
    ...
    def recv(self, stream, crc_mode=1, retry=16, timeout=60, delay=1, quiet=0):
        ...
        while True:
            ...
            if error_count >= retry:
                print('error_count reached %d, aborting.', retry)
                self.abort(timeout=timeout)
                return None
            elif crc_mode and error_count < (retry // 2):
                if not self.putc(CRC):
                    print('recv error: putc failed, '
            ...
            char = self.getc(1, timeout)
            if char is None:
                print('recv error: getc timeout in start sequence')
                error_count += 1
                continue
            elif char == SOH:
                print('recv: SOH')
                ...
                if cancel:
                    print('Transmission canceled: received 2xCAN '
                                  'at start-sequence')
                    return None
                else:
                    print('cancellation at start sequence.')
                    cancel = 1
            else:
                error_count += 1
        ...
        while True:
            while True:
                if char == SOH:
                    if packet_size != 128:
                        print('recv: SOH, using 128b packet_size')
                        packet_size = 128
                    break
                elif char == STX:
                    if packet_size != 1024:
                        print('recv: SOH, using 1k packet_size')
                        packet_size = 1024
                    break
                elif char == EOT:
                    ...
                    if cancel:
                        ...
                        print('cancellation at block %d', sequence)
                        cancel = 1
                else:
                    ...
                    char = self.getc(1, timeout)
                    if error_count >= retry:
                        print('error_count reached %d, aborting.',
                                      retry)
                        self.abort()
                        return None
            ...
            if seq1 is None:
                print('getc failed to get first sequence byte')
                seq2 = None
            else:
                seq1 = ord(seq1)
                seq2 = self.getc(1, timeout)
                if seq2 is None:
                    print('getc failed to get second sequence byte')
                else:
            ...
            print('recv error: purge, requesting retransmission (NAK)')
            while True:
    ...
    def _verify_recv_checksum(self, crc_mode, data):
        if crc_mode:
            try:
                ...
                if not valid:
                    print('recv error: checksum fail '
                                  '(theirs=%04x, ours=%04x), ',
                                  their_sum, our_sum)
            except:
                return False, data
